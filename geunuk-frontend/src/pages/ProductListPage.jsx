...
export default function ProductListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (category !== '전체') products = products.filter(product => product.category === category);
  ...
  return (
    <div className="page-wrap">
      ...
      <header className={styles.heading}><p>SHOP</p><h1>{category === '전체' ? '전체 상품' : category}</h1></header>
      ...
      <section className={styles.filters}>
        <div className={styles.categories}>{CATEGORIES.map(item => <button key={item} className={category === item ? styles.selected : ''} onClick={() => setFilter('category', item)}>{item}</button>)}</div>
